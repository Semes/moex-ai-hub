...
Search specific domains using the CLI tool below. **Run all commands from the project repository root.**
...
```
On Windows, use `py` instead of `python` in commands if `python` is not in PATH.
